...
| `docs/test-strategy/ux-api-contract.md` | UX/a11y contract the asoe-ui binds against — `error.message` is the aria-live announcement, display-safe enum labels in `useHealth()`, no inline HTML in display fields. Companion to `asoe-ui/docs/test-strategy/UX_ACCESSIBILITY.md`. Locked by `tests/contract/test_error_envelope_ux_contract.py` (AST walk over every `raise ASOEError(...)` site: code+message kwargs present; string-literal messages non-empty + end with terminal punctuation, with f-string trailing-Constant handled; no code→message echo). |
