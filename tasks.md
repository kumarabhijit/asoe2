...
- [x] **Attachment store (DoR #10 residue)** (2026-05-25) — production attachment store = ASOE's own DB (V016 `email_attachment`, SQLite/Postgres) via `gateways/attachment_store.py` (tenant-scoped, SHA-256, oversize guard); `store_backed_fetcher` injected into `AttachmentFetchGateway` behind the SSRF allowlist; returns FAILED (not a crash) on fetch error (`tests/test_attachment_store.py`)

## PARITY — Azure pre-prod ↔ Vercel dev parity (per `docs/plans/azure-preprod-parity-plan.md`)

Source-of-truth status table lives at the top of the parity plan. The
checklist below mirrors the canonical view; updates land in both
places per `prompts/update_docs.md`.

- [x] **PARITY-0** Sandbox-stub Azure pre-prod + fail-loud boot — `api/preprod_gateways.py`, `api/production_gateways.py`, CORS regex validation, Postgres `connect_timeout`, bicep VNet/private-Postgres path commented, image-scan + secret-scan + dep-audit CI gates (advisory), non-root verified, secrets-rotation runbook, 9-test preprod registration suite.
- [x] **PARITY-0.5** Audit-chain tombstone routing — `erase_attachment` writes the `ATTACHMENT_ERASED` row BEFORE byte wipe; `GET /api/v1/attachments/{id}/erasure-certificate` returns the chain-proof tombstone (manager+admin, tenant-scoped); `AttachmentErasureTombstone` schema locked in the audit-bearing registry; PII-free invariant (no `content`, no `name`).
- [x] **PARITY-3a** Frontend NextAuth dual-provider scaffold — see asoe-ui tasks.md.
- [x] **PARITY-3b** Backend Entra JWKS + role mapping — `api/azure_ad_jwks.py` 1h TTL cache + kid-rotation refresh + fail-closed, `api/azure_ad_roles.py` group→role, `api/refresh_token_revocation.py` jti index, `api/deps.py::_entra_decode` with aud/iss/kid/exp pinning, cross-tenant 403 regression test.
- [x] **PARITY-4** Key Vault + separate signing keys — `infra/main.bicep` Key Vault RBAC + 90d soft-delete + purge-protection; `api/attachment_read_token.py` reads `ASOE_ATTACHMENT_SIGNING_KEY` with SECONDARY rotation slot; `docs/ops/secrets-rotation.md`.
- [x] **PARITY-5** App Insights + OTel + `@audit_bearing` — `api/observability/otel.py` opt-in init, `audit_bearing.py` decorator + multi-line lint, `compliance/audit_bearing_exemptions.yaml` grandfather list, `api/observability/alerts.py` 7 pre-declared alerts (including `extraction-drift` Sev3), `api/observability/log_redaction.py` PII scrubber. Last 3 grandfather migrations shipped: `/signed-url`, `/metrics/reviewer-activity`, `/outbox/reconcile`.
- [x] **PARITY-6** Real-connector scaffolding — `contracts/policy.py::GATEWAY_TIMEOUT_S` per-gateway budgets; `api/dead_letter_queue.py` tenant-scoped DLQ; `gateways/azure_di_egress_redaction.py` Luhn-checked CC + SSN + IBAN scrubber; `tests/eval/shadow_mode_thresholds.yaml` Q9 thresholds; `docs/ops/fixture-capture.md` cadence + sanitisation checklist.
  - [x] **6.1** Graph `email_intake` — `gateways/msgraph_intake.py::GraphIntakeGateway` + `LiveGraphIntakeBackend` behind `ASOE_EMAIL_INTAKE_DRIVER=graph`; ShadowRunner-gated; `ASOE_CANARY_PCT_EMAIL_INTAKE`; terminal failure → DLQ + stub fallback; `fetch_message` body egress through `redact_for_azure_di`. CHANGELOG at `gateways/changelog/email_intake.md`.
  - [x] **6.2** AzureDI `document_extraction` — `LiveDocumentIntelligenceBackend` + `resolve_backend()` selector behind `ASOE_DOCUMENT_EXTRACTION_BACKEND=live`; constructor refuses without endpoint+key/MI; `extract_anchors` records `record_extraction_drift` per call.
  - [x] **6.3** S/4HANA SAP (7 domains) — `gateways/sap_live.py::SapDomainGateway` + shared `LiveSapBackend` behind `ASOE_SAP_DRIVER=s4hana`; one wrapper per domain (sap_order, sap_doc, sap_contract, promotion, sap_block, sap_customer_master, sla_contract); DLQ source `"sap"` + `domain` payload field for the operator-dashboard pivot. CHANGELOG at `gateways/changelog/sap.md`.
  - [x] **6.4** OMS — `gateways/oms_live.py::OmsGateway` + `LiveOmsBackend` behind `ASOE_OMS_DRIVER=live`; `record_post_success_orphan` helper wired into `orchestration/nodes.py::apply_effects` so a future OMS write effect's failure auto-fires the orphan surfacing. CHANGELOG at `gateways/changelog/oms.md`.
  - [ ] **Live HTTP transport for each backend** — gated to nightly `-m live` mark; each `LiveXxxBackend.execute` / `propose` raises `NotImplementedError` until the platform team fills in the body with real credentials.
- [x] **PARITY-7** Spatial extraction hardening — `_normalize` NFKC + soft-hyphen; `resolve_model_id` env-pin; golden set expanded 1 → 12 rows; per-doc-type thresholds; Brier calibration test. Drift forwarder Job (`scripts/run_drift_forwarder.py` + `infra/main.bicep::driftAlertJob`) shipped.
- [x] **PARITY-8** Data governance closure — `api/retention_sweeper.py` kill-switch + dry-run + residency-check + identity resolution + **real byte-wipe via `erase_attachment` with `SCHEDULED_RETENTION_DELETE` distinct audit row per candidate**; per-tenant TTL; DPIA template; erasure-flows doc; `scripts/run_retention_sweeper.py` Container Apps Job CLI; `infra/main.bicep::retentionSweeperJob` cron Job gated on `deployRetentionSweeperJob` + `RETENTION_SWEEPER_ENABLED` two-step opt-in.
