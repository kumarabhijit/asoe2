...
### 3.4 The in-flight branch's adaptation requirement (estimated 30-60 min when it merges)

When `claude/review-order-entry-architecture-RCIUa` rebases onto main after this branch merges:

1. `git mv skills/email-order-entry_SKILL.md knowledge/skills/email-order-entry/SKILL.md` (mechanical).
2. Create `knowledge/skills/email-order-entry/metadata.yaml` matching the pattern Phase H.1 establishes for the other skills.
3. Update any in-flight tests that reference the old path (`tests/sandbox/test_recipe_integrity.py` and `tests/test_skills.py` if it exists) — search-and-replace one path string.
4. Empty `examples/`, `assets/`, `specs/` directories per ADR-038 §5.5 (examples are *earned* by real failures, not authored speculatively).
5. The PO spec `docs/specs/order-entry-from-email-product-spec.md` migrates to `knowledge/skills/email-order-entry/specs/order_entry_spec.md` (per the user's request to co-locate raw specs with skills).

This is documented here so the in-flight branch's reviewer has the migration steps in front of them at rebase time.
