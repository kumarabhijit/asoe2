...
| `ASOE_EMBEDDING_PROVIDER` + `ASOE_EMBEDDING_MODEL` | O (unset → correlate fallback) | O (bicep `embeddingProvider`/`embeddingModel`) | unset → precedents use the deterministic correlate match; advisory retrieval only, never a control field. Requires `OPENAI_API_KEY` (compose passthrough / bicep `openai-api-key` secret, rotatable via set-secrets.sh) when set to `openai` |
