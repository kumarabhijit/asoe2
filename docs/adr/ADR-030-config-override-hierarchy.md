...
**Applies to:** `gateways/tenant_config.py` (new), `db/migrations/V006__tenant_config.sql` (new), `api/routes/policies.py`, new `api/routes/config.py`, `contracts/models.py` (`ConfigChange` event), `contracts/policy.py` (platform defaults stay; per-tenant resolution moves to gateway), `tests/test_tenant_config_gateway.py`.
...
CREATE TABLE tenant_config (
    ...
    intent          VARCHAR(40) NOT NULL,           -- e.g., 'DUPLICATE_PO'
    layer           VARCHAR(20) NOT NULL,           -- 'TENANT' | 'TIER' | 'CUSTOMER' | 'CHANNEL'
    -- Layer scope keys (NULL where N/A for the layer):
    customer_tier   VARCHAR(20),                    -- 'strategic' | 'standard' | 'smb' (TIER, CUSTOMER, CHANNEL)
    customer_id     UUID REFERENCES customers(id),  -- (CUSTOMER, CHANNEL)
    channel         VARCHAR(20),                    -- 'EDI' | 'API' | 'EMAIL' | 'PORTAL' (CHANNEL only)
    -- Payload:
    config          JSONB NOT NULL,                 -- partial override map
    -- Sandbox vs prod:
    environment     VARCHAR(10) NOT NULL DEFAULT 'production',  -- 'production' | 'sandbox'
    -- Versioning + audit:
...
For weight maps specifically, the merge follows ADR-029 (key-by-key override; sum-to-1 validated after full resolution; fail-closed to L1 on violation).
...
GET    /api/v1/config/:intent/resolve?tenant_id=&customer_id=&customer_tier=&channel=&environment=
       -> { resolved: {...}, trace: [...] }     # admin diagnostic, U3 ask
...
class ConfigChange(BaseModel):
    ...
    tenant_id: UUID
    intent: Intent
    layer: ConfigLayer  # ENUM
    scope: dict          # tier/customer_id/channel as applicable
    environment: ConfigEnvironment  # ENUM
    operation: ConfigOperation       # CREATE | UPDATE | DELETE | PROMOTE
    before: dict | None
    after: dict | None
    actor_id: UUID
    actor_role: str
...
- **Hierarchy honesty (DDD):** Each level is a distinct authority — platform engineers own L1, tenant admins own L2, customer-success owns L3 + L4, integration owns L5. Modeling them as one flat config-blob would conflate authorities.
...
2. `gateways/tenant_config.py::resolve_for_event` implementing the 5-level merge + trace; backed by `tenant_config` table for L2–L5, `contracts/policy.py` for L1.
3. `contracts/models.py` — add `ConfigChange`, `ConfigLayer`, `ConfigEnvironment`, `ConfigOperation`.
...
1. Admin UI (in `asoe-ui`) for L4 (customer-specific) and L5 (customer-channel) editing with sandbox-promote workflow.
2. `customer_behavior_overrides` (blanket_po, drop_ship, high_frequency from spec) as named presets the admin can apply to a customer at L4 — these materialize into normal CUSTOMER-layer rows; they are not a sixth layer.
3. CS-Manager-facing autonomy-tuning UI (write a customer-specific override that adjusts only `autonomy_levels`).
...
- L4 and L5 in V1 require API-level admin work (curl/Postman). Acceptable per U3 in design review; flagged in V1.5 backlog.
- `ConfigChange` events grow the audit chain. Volume estimate at V1 scale: <100 changes/tenant/month — negligible.
...
- Cache invalidation strategy on `ConfigChange`: process-local TTL cache vs. pub/sub invalidation. V1 ships with TTL (60s) for simplicity; pub/sub when latency budget pressure justifies it.
