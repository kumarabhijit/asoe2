...
Split `duplicate_check_results` into a dedicated **read projection** (materialized view first, escalated to a physical table only if MV is insufficient) when **any** of the following conditions hold for **two consecutive weeks**:

### Trigger conditions (any one of these, sustained 2 weeks)

| # | Condition | Measurement source | Threshold |
|---|---|---|---|
| T1 | P95 latency on `GET /api/v1/exceptions/duplicates` (list endpoint) breaches budget | `observability/metrics.py` Prometheus histogram | **> 800 ms** |
| T2 | P95 latency on `GET /api/v1/exceptions/duplicates/:id` (envelope endpoint, ADR-028 Guard-rail 2) breaches budget | Same | **> 400 ms** |
| T3 | Duplicate-PO query share of total exception-route DB time | pg_stat_statements + intent tagging | **> 30%** |
| T4 | Number of GIN indexes required on `ExecutionLog.recipe_output` to keep T1/T2 within budget | `db/migrations/` count + EXPLAIN ANALYZE plans | **> 2 indexes** |
| T5 | Calibration work is officially scheduled (proactive trigger from ML lens) | Roadmap / ticket marked "calibration kickoff" | **scheduled within next 90 days** |

The "two consecutive weeks" requirement filters transient spikes (a single bad week from a backfill or an outage doesn't trigger re-architecture).
...
- **T1 / T2 (latency):** the UI's two hot paths. The user-experience SLOs in the spec (queue load, Layer-2 detail in one fetch) ride directly on these.
- **T3 (query share):** even if absolute latency is fine, a single intent dominating DB time disproportionately means JSONB indexes are doing too much work and a typed projection would relieve them.
...
- **T5 (calibration scheduled):** ML lens's explicit condition for accepting unified V1. Pulling labeled training pairs out of nested JSONB across 10M rows is painful; doing it under deadline pressure is worse. T5 lets us land the projection *before* it becomes urgent.
- **Two-week sustainment:** standard SRE practice for trigger-based decisions. Avoids reactive rework on transient spikes.
...
3. Weekly automated report: a small script that emits the current values of T1–T4 to a known dashboard / wiki page, so the trigger conditions are visible without anyone having to remember to look.
...
| **Tighter latency thresholds (e.g., T1 > 500ms)** | Premature optimization at V1 volume. The 800ms / 400ms numbers reflect what users will notice; tighter than that is engineering vanity. |
| **No two-week sustainment requirement** | Single-week spikes (e.g., a backfill) would trigger evaluation. Wastes the team's time on transient signals. |

---

## Open questions

- Whether T3's "30% of DB time" should be measured tenant-wide or platform-wide. Current decision: platform-wide (so a single noisy tenant's queries can trigger if they dominate). Revisit if multi-tenant skew becomes pathological.
