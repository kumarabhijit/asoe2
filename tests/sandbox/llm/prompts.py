...
def recipe_prompt(intent: str) -> str:
    ...
    return (
        ...
        "  - BackOrderResolutionRecipe.py   (for BACK_ORDER)\n"
        "  - OverMaxTrimRecipe.py           (for OVER_MAX)\n"
        "  - MOQRoundUpRecipe.py            (for MIN_ORDER_QTY)\n"
        "  - PalletAlignmentRecipe.py       (for PALLET_CONFIG)\n"
        "  - DeliveryDelayResolutionRecipe.py (for DELIVERY_DELAY)\n"
...
def shadow_prompt(intent: str, line_count: int, batch_variance: float) -> str:
    ...
    return (
        ...
        "  - BACK_ORDER (gap ≥ 50%)                → RED (escalate to buyer)\n"
        "  - BACK_ORDER (gap < 50%)                → YELLOW (review options)\n"
        "  - OVER_MAX (exceedance > 50%)           → RED (sales-manager)\n"
        "  - OVER_MAX (exceedance ≤ 50%)           → YELLOW (trim review)\n"
        "  - MIN_ORDER_QTY (shortfall ≥ 25%)       → RED (KNMT waiver)\n"
        "  - MIN_ORDER_QTY (shortfall < 25%)       → YELLOW (round-up review)\n"
        "  - PALLET_CONFIG (violation present)     → YELLOW (alignment review)\n"
        "  - DELIVERY_DELAY (≥5 days late)         → RED (severe)\n"
        "  - DELIVERY_DELAY (2–4 days late)        → YELLOW (minor review)\n"
        "  - otherwise                             → GREEN\n\n"
